    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if is_available:
                ...
                logger.warning(
                    f"Ollama model '{model_name}' is not available. Available models: {available_models}",
                )
            return is_available
        logger.warning(
            f"Could not check Ollama models, status code: {response.status_code}",
        )
        return False
...
class OllamaDocumentProcessor(DataComponent):
    ...
    def __call__(self, documents: Sequence[Document]) -> Sequence[Document]:
        ...
        logger.info(
            f"Processing {len(output)} documents individually for Ollama embeddings",
        )
        ...
        for i, doc in enumerate(
            tqdm(output, desc="Processing documents for Ollama embeddings"),
        ):
            try:
                ...
                if result.data and len(result.data) > 0:
                    ...
                    if expected_embedding_size is None:
                        ...
                        logger.warning(
                            f"Document '{file_path}' has inconsistent embedding size {len(embedding)} != {expected_embedding_size}, skipping",
                        )
                        continue
        ...
        logger.info(
            f"Successfully processed {len(successful_docs)}/{len(output)} documents with consistent embeddings",
        )
