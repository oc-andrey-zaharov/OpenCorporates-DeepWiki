...
async def get_github_repo_structure(
    ...
    try:
        ...
        # Prepare headers with GITHUB_TOKEN if available
        headers = {"Accept": "application/vnd.github.v3+json"}

        if GITHUB_TOKEN:
            headers["Authorization"] = f"Bearer {GITHUB_TOKEN}"
            logger.info("Using GITHUB_TOKEN for repository access")
        else:
            logger.warning(
                "GITHUB_TOKEN not found in environment. Requests may fail for private repositories."
            )
        ...
        for branch in branches_to_try:
            try:
                api_url = (
                    f"{api_base}/repos/{owner}/{repo}/git/trees/{branch}?recursive=1"
                )
        ...
        if not tree_data or "tree" not in tree_data:
            ...
            if not GITHUB_TOKEN:
                error_msg += " GITHUB_TOKEN not found in environment. Please add it to your .env file."
        ...
        return JSONResponse(
            status_code=500,
            content={"error": f"Error fetching repository structure: {str(e)}"},
        )
