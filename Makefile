...
dev/backend:
	...
	@mkdir -p api/logs
	@poetry run python -m api.server.main > api/logs/server.log 2>&1 & \
		SERVER_PID=$$!; \
		...
		echo "  Server logs: tail -f api/logs/server.log"
	...
	@if [ -f .deepwiki-server.pid ]; then \
		...
		if ps -p $$PID > /dev/null 2>&1; then \
			...
			if echo "$$CMD" | grep -q "api.server.main"; then \
				...
				echo "PID file exists but process does not match expected backend. Using port-based fallback."; \
