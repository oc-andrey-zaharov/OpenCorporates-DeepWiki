...
def _delete_via_server(selected_wiki: dict) -> None:
    """Delete wiki via server API.

    Args:
        selected_wiki: The wiki dictionary to delete

    Raises:
        click.Abort: If deletion fails
    """
...
def delete(yes: bool) -> None:
    ...
    if is_server_mode():
        _delete_via_server(selected_wiki)
    else:
        _delete_standalone(selected_wiki)


def _delete_standalone(selected_wiki: dict) -> None:
